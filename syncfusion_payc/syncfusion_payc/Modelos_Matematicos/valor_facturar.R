factura <- function(cod_factura) {
  ...
  if (any(CONDICIONES_CONTRATO$COD_TIPO_CONDICION == 4)) {
    if (nrow(NOVEDADES_DESCUENTO) != 0) {
      ...
      NOVEDADES_DESCUENTO$HORAS_DESCUENTO[NOVEDADES_DESCUENTO$HORAS_DESCUENTO>240] <-240
    }} else {
  ...
  eliminar <- paste0("DELETE FROM [dbo].[DETALLE_FACTURA_PERS] WHERE [COD_CONTRATO_PROYECTO] =", proyecto, "AND COD_FORMAS_PAGO_FECHAS=", fecha, "AND COD_FACTURA=",cod_factura)
  dbExecute(con, eliminar)
  eliminar <- paste0("DELETE FROM [dbo].[DETALLE_FACTURA_ITEM] WHERE [COD_CONTRATO_PROYECTO] =", proyecto, "AND COD_FORMAS_PAGO_FECHAS=", fecha, "AND COD_FACTURA=",cod_factura)
  dbExecute(con, eliminar)
  eliminar <- paste0("DELETE FROM [dbo].[DETALLE_FACTURA_ADJUNTO_PERS] WHERE [COD_CONTRATO_PROYECTO] =", proyecto, "AND COD_FORMAS_PAGO_FECHAS=", fecha, "AND COD_FACTURA=",cod_factura)
  dbExecute(con, eliminar)
  eliminar <- paste0("DELETE FROM [dbo].[DETALLE_FACTURA_ADJUNTO_ITEM] WHERE [COD_CONTRATO_PROYECTO] =", proyecto, "AND COD_FORMAS_PAGO_FECHAS=", fecha, "AND COD_FACTURA=",cod_factura)
  dbExecute(con, eliminar)
