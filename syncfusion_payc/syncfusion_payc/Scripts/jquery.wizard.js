	...
	methods = {
		init: function (element, options) {
			...
			if(opts.topButtons && stepsBar.length && !topActions.length){
				...
				html += '<span class="btn btn-default btn-prev" onclick="validar_intervencion($(this))"><span class="previous-text">&nbsp;&nbsp;&nbsp;' + opts.text.previous + '</span></span>';
				html += '<span class="btn btn-default btn-next" onclick="validar_intervencion($(this))"><span class="next-text">&nbsp;&nbsp;&nbsp;' + opts.text.next + '</span><span class="finished-text">' + opts.text.finished + '</span></span>';
				html += '</div>';
			...
			for(var i=0;i<stepsLi.length;i++){
				...
				content = '<br><span class="step-text">'+ step.html() +'</span>';
				
				step.empty().html('<span class="img-circle step-index"><span class="percent percent-'+ii+'">0%</span></span>'+ content + '');
